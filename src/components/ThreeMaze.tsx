...
import { EXIT, KEY_CELLS, SIZE, walls } from '../lib/game'
...
export function ThreeMaze({ game, color }: Props) {
  ...
  useEffect(() => {
    ...
    const lamp = new THREE.PointLight(0xe98566, 48, 18)
    ...
    const keyMeshes = KEY_CELLS.map((cell) => {
    ...
    const draw = () => {
      ...
      door.material.emissive.setHex(current.keys.length === KEY_CELLS.length ? 0x8c3a16 : 0x210504)
      frame += 1
